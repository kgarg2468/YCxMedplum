...
async function prepareReview(
  ...
  const review = runReview({
    ...
    redFlags: [...checkRedFlags(transcript), ...extraction.red_flags],
...
async function runPipeline(
  ...
  try {
    ...
    const prepared = session?.preparedReview ?? await prepareReview(deps, transcript, session);
...
export function createWebhookApp(deps: ServerDependencies): Express {
  ...
  app.post('/vapi', async (req, res) => {
    ...
    if (msg?.type === 'transcript' && msg.role === 'user') {
      ...
      if (flags.length) void escalateRedFlags(deps, flags, msg.call?.id);
      return;
    }
